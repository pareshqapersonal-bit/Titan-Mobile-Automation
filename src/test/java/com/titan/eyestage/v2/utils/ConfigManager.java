...
public class ConfigManager {
    ...
    // v2 is the BrowserStack parallel-device flow, so default here (unlike the
    // original flow's ConfigManager) is browserstack, not local. Still
    // overridable via -DexecutionMode=local or the EXECUTION_MODE env var.
    private static final String DEFAULT_EXECUTION_MODE = "browserstack";
    ...
    private void loadConfiguration() {
        ...
        try {
            ...
            // v2 keeps its own base config (Config-v2.properties), separate
            // from the original flow's Resources/Config.properties, so its
            // default execution mode (browserstack) never depends on or
            // affects the original flow's file.
